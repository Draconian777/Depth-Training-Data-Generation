...
#Delete all cubes and create a new template
...
def renderImage(index,cam,scene,path):
...
def renderDepth(index,cam='D',scene=0,path="E:/OneDrive/Computer Engineering MSc/Thesis/Blender Files/Scripting/DataOutput/"):
    ...
    im_path= "%sScene%sIm%s%s.png"%(path,str(scene),str(index),cam) 
...
baseLine=1
totalScenes=1  #Total number of scenes to generate 
...
moveBase(0.5)
#aimCam()
renderDepth(100,'D',99,path)
returnStart()


bpy.context.scene.render.engine = 'BLENDER_WORKBENCH'
renderImage(100,'L',99,path)
moveBase(1)
renderImage(100,'R',99,path)
returnStart()